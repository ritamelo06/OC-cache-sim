...
void accessL2(uint32_t address, uint8_t *data, uint32_t mode) {
  ...
  Tag = address >> 15;                // remove index + offset
  offset = address & 0x3F;            // 0x0011 1111 
  Index = (address >> 6) & 0x1FF;     // remove offset and tag; 0x1FF = 0x0001 1111 1111
  MemAddress = address >> 6;          // remove offset
  MemAddress = MemAddress << 6;       // address of the block in memory

  CacheLine *Line = &cacheL2.lines[Index];     // get line from cache
  ...
  if (!Line->Valid || Line->Tag != Tag) {
    ...
    // line has dirty block
    if ((Line->Valid) && (Line->Dirty)) {        
      MemAddress = (Line->Tag << 15) + (Line->Index << 6);  // get address of the block in memory
